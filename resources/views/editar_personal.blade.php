...
@section('content')
<!-- Lorem20   -->
<form class="w-full max-w-lg mx-auto mt-5" action="{{route('personal.update', $use->id) }}" method="POST">
  @csrf
  ...
  <div class="flex flex-wrap -mx-3 mb-6">
        <div class="w-full px-3">
            ...
            <input name="name" value="{{ $use->name }}" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-zip" type="text" placeholder="Name">
  ...
  <div class="flex flex-wrap -mx-3 mb-2"> <!-- 3 Campos -->
    <div class="w-full md:w-2/3 px-3 mb-6 md:mb-0"> <!--  correo    -->
      ...
      <input name="email" value="{{ $use->email }}" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-zip" type="email" placeholder="email">
      ...
      <div class="relative">
        <select name="dependencia_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" >
          ...
          @foreach ($depen as $depe)
            <option value="{{ $depe['id'] }}" @if($use->dependencia_id == $depe['id']) selected ='selected'; @endif>{{ $depe['nombredepen'] }}</option>
          @endforeach
        </select>
